...
async function addToBatch(opDid, devDid, action) {
  ...
  if (buffer.length >= BATCH_SIZE) {
    ...
    buffers.set(key, []); // Clear the buffer after sending the batch
  }
}
...
// -----------------------------
// SEQUENTIAL RUN (with batch sending)
// -----------------------------
export async function runSequentialFull() {
  ...
  // Sequentially run each operation
  ...
  // Process any remaining transactions (those that did not hit 50 actions but still need to be sent)
